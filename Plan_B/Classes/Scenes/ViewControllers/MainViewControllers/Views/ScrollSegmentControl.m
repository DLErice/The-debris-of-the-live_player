...
@implementation ScrollSegmentButton
...
@interface ScrollSegmentControl ()
{
    ...
    ScrollSegmentButton *_currentBtn;   // 指示视图当前所在的按钮
...
//    _titleColor = titleColor;
    ...
    for (int i = 0; i < _btnCount; i++) {
        ScrollSegmentButton *btn = self.buttons[i];
...
- (void)setBackgroundColor:(UIColor *)backgroundColor
{
    ...
    [_currentBtn setTitleColor:[UIColor colorWithRed:0.843 green:0.651 blue:0.235 alpha:1.000] forState:UIControlStateNormal];
}
...
- (void)createUI
{
    ...
    // 指示视图
    _indicatorView = [[UIView alloc] initWithFrame:CGRectMake(0, btnHeight-4 , _btnWidth, 3)];
...
//    _indicatorView.layer.masksToBounds = YES;
    ...
    for (int i = 0; i < _btnCount; i++)
    {
        ScrollSegmentButton *btn = [ScrollSegmentButton buttonWithType:UIButtonTypeSystem];
    ...
    // 第一个设置字体颜色
    _currentBtn = (ScrollSegmentButton *)self.buttons[0];
    // 如果不设置backgroundColor，第一个按钮的文字颜色就会被设置为默认颜色
    [_currentBtn setTitleColor:[UIColor colorWithRed:0.843 green:0.651 blue:0.235 alpha:1.000] forState:UIControlStateNormal];
}
...
- (void)setSelectedIndex:(NSInteger)index
{
    ...
    [UIView animateWithDuration:0.25f animations:^{
        ...
        _indicatorView.frame = CGRectMake(_btnWidth * index, self.frame.size.height-4, _indicatorView.frame.size.width, _indicatorView.frame.size.height);
...
//    UIColor *color = self.backgroundColor ? self.backgroundColor : [UIColor blackColor];
    [_currentBtn setTitleColor:[UIColor colorWithRed:0.843 green:0.651 blue:0.235 alpha:1.000] forState:UIControlStateNormal];
}
